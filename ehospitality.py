...
from .forms import PrescriptionForm, AppointmentForm
from .models import Patient, MedicalHistory, TreatmentPlan, Doctor, Prescription, CustomUser, Facility, \
    BillingStatement, InsuranceInfo
from .models import Patient, Appointment
from .models import Patient
from final_project.models import Patient
...
def cancel_appointment(request, appointment_id):
    ...
    if request.method == 'POST':
        ...
        appointment.save()
        return render(request, 'cancel_success.html')  # ✅ Must return a valid response
...
#     billing = get_object_or_404(BillingStatement, patient=patient)
...
def load_data(file):
    if not os.path.exists(file):
        with open(file, 'w') as f:
            json.dump([], f)  # ✅ Create empty list, not dict
    with open(file, 'r') as f:
        return json.load(f)
...
def billing_view(request):
    if request.method == 'POST':
        ...
        billing_data = load_data(BILLING_FILE)  # Should return a list, NOT a dict

        billing_data.append(data)  # ✅ This works only if billing_data is a list
